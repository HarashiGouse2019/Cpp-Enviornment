...
#define KEY_UP 72 //ASCII CODE FOR THE UP KEY
#define KEY_DOWN 80 //ASCII CODE FOR THE DOWN KEY
#define KEY_LEFT 75 //ASCII CODE FOR THE LEFT KEY
#define KEY_RIGHT 77 //ASCII CODE FOR THE RIGHT KEY
#define SPACE 32 //ASCII CODE FOR THE SPACE BAR
#define RETURN 13 //ASCII CODE FOR THE ENTER/RETURN KEY
#define ALT_UNIT Beep(1000, 50); Beep(1000, 50); //BEEPS TWO TIMES WITH A FREQUENCY OF 1000 AND A DURATION OF 50 MILLISECONDS
#define SHOW_POS Beep(500, 50); Beep(500, 50); //BEEPS TWOS TIME WITH A FREQUENCY OF 500 AND A DURATION OF 50 MILLISECONDS
#define MATCH Beep(2000, 200); //BEEPS ONE TIME WITH A FREQUENCY OF 2000 AND A DURATION OF 200 MILLISECONDS


//Create a class called Sector
class Sector {
public:
	unsigned int sectorSize = 1000; //The Default Sector Size
	...
	while (1)
	{
		...
		case KEY_UP:
			...
			*ptry += unitSize; //MoveUp
			Beep((max(*ptry, enemy.coord_y) - min(*ptry, enemy.coord_y)) * 10, 100); //Send a sound to let us know that we are getting closer
			break;
		...
		case KEY_LEFT:
			...
			*ptrx -= unitSize; //MoveLeft
			Beep((max(*ptrx, enemy.coord_x) - min(*ptrx, enemy.coord_x)) * 10, 100); //Send a sound to let us know that we are getting closer
			break;
